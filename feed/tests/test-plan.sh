...
[ -z "$(jq -c '.cells[]|select(.id=="opkg-noarch-luci")' <<<"$p")" ] || { echo "formats must keep dashboard out of opkg"; exit 1; }
...
[ "$(jq -r '[.cells[]|.id]|length' <<<"$p")" = 7 ] || { echo "7 cells: $(jq -c '[.cells[].id]' <<<"$p")"; exit 1; }
